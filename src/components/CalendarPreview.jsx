...
export default function CalendarPreview({ tasks, printPreview = false, includePrintNotes = false }) {
  ...
  return (
    <>
      ...
      {printPreview && includePrintNotes && (
        <div className="mt-8 border border-theme-border/30 dark:border-white/10 rounded-xl p-6 print:border-black print:p-4">
          <h3 className="font-serif font-semibold text-sm mb-4 text-theme-text print:text-black">
            Notes & Hand-written Tasks
          </h3>
          <div className="flex flex-col gap-6">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="border-b border-dashed border-theme-border/40 print:border-black/40 h-8 flex items-end"
              >
                <span className="text-[10px] text-theme-text-muted/40 mb-1 select-none print:hidden">
                  Custom annotation line {i + 1}
                </span>
              </div>
            ))}
          </div>
